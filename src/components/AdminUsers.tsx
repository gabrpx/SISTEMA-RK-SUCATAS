...
export default function AdminUsers({ userRole, onModalChange, theme = 'dark' }: { userRole?: string, onModalChange?: (isOpen: boolean) => void, theme?: 'light' | 'dark' }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // A resposta base da nossa API via Express res.json(data) desce diretamente como array caso sucesso
      const allUsers = Array.isArray(response) ? response : (response.data || []);

      // Staff are users with roles other than 'client' or 'cliente'
      const staffUsers = allUsers.filter((u: any) => u.cargo && u.cargo !== 'client' && u.cargo !== 'cliente').map((u: any) => ({
        ...u,
        role: u.cargo,
      ...
      const authClients = allUsers.filter((u: any) => !u.cargo || u.cargo === 'client' || u.cargo === 'cliente').map((u: any) => ({
        ...u,
        role: u.cargo || 'client',
        name: u.nome || 'Sem nome',
        phone: u.telefone || 'Sem telefone',
  ...
  const handleMigrateFirebaseData = async () => {
    if (!window.confirm("Isso fará o download de todos os seus contatos que ficaram no antigo Firebase e irá inseri-os no Supabase atual.\n\nTem certeza que deseja processar a recuperação?")) return;
    ...
    try {
      ...
      for (const docSnap of usersSnap.docs) {
        ...
        const role = data.role === 'client' ? 'cliente' : (data.role || 'cliente');
        try {
          await api.post('/api/usuarios', {
            nome: data.name || data.nome || 'Sem nome',
            email: data.email || undefined,
            telefone: data.phone || data.telefone || undefined,
            cargo: role,
            interesses: data.interests ? JSON.stringify(data.interests) : undefined,
            compras: data.purchases ? JSON.stringify(data.purchases) : undefined
          });
          count++;
        } catch (e) {
          console.error("Erro ao migrar usuário:", data.email, e);
        }
      }
      ...
      for (const docSnap of clientsSnap.docs) {
        ...
        try {
          await api.post('/api/usuarios', {
            nome: data.name || data.nome || 'Cliente Anônimo',
            telefone: data.phone || data.telefone || 'Sem telefone',
            cargo: 'cliente',
            interesses: data.interests ? JSON.stringify(data.interests) : undefined,
            compras: data.purchases ? JSON.stringify(data.purchases) : undefined
          });
          ...
          console.error("Erro ao migrar cliente:", data.phone, e);
        }
      }
      
      alert(`Recuperação concluída com sucesso! ${count} registros resgatados para o Supabase.`);
      fetchData();
    } catch (err: any) {
  ...
  const handleToggleRole = async (userId: string, currentRole: string) => {
    const roles = ['admin', 'gerente', 'estoque', 'cliente'];
    // Tratamos client/cliente como mesmo escopo numérico
    const normalizedRole = currentRole === 'client' ? 'cliente' : currentRole;
    ...
    try {
      await api.put(`/api/usuarios/${userId}`, { cargo: newRole });
      
      if (newRole === 'cliente') {
        const userToMove = users.find(u => u.id === userId);
        if (userToMove) {
          setUsers(users.filter(u => u.id !== userId));
          setClients([...clients, { ...userToMove, role: 'cliente', phone: userToMove.phone || 'Sem telefone' } as any]);
        }
...
if (!window.confirm(`ATENÇÃO: Tem certeza que deseja excluir permanentemente o usuário ${userName || userId}?`)) return;
  ...
  const handleSaveClient = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        telefone: clientFormData.phone,
        nome: clientFormData.name,
        cargo: 'cliente',
        interesses: JSON.stringify(clientFormData.interests),
        compras: JSON.stringify(clientFormData.purchases),
        senha: clientFormData.password || undefined // Idealmente só salvariamos no Supabase Auth. Para o legado vamos ignorar.
      };

      if (editingClient) {
        await api.put(`/api/usuarios/${editingClient.id}`, payload);
        const updatedClient = {
          ...editingClient,
          phone: payload.telefone,
          name: payload.nome,
          interests: payload.interesses,
          purchases: payload.compras,
          role: payload.cargo
        };
...
if (!window.confirm(`ATENÇÃO: Tem certeza que deseja excluir permanentemente o cliente ${clientName || clientId}?`)) return;
    ...
    try {
      await api.delete(`/api/usuarios/${clientId}`);
  ...
  return (
    <div className="space-y-10 relative">
      <div className="absolute top-0 right-0 z-10 w-full flex justify-end px-4">
        <button 
          onClick={handleMigrateFirebaseData} 
          className="text-xs bg-red-600/20 hover:bg-red-600/40 text-red-400 border border-red-500/30 px-3 py-1.5 rounded flex items-center gap-2 transition-all"
        >
