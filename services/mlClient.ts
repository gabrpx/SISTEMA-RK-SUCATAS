...
class MLClient {
   ...
   */
  async ensureUserId() {
    if (!this._userId || this._userId === 'undefined' || this._userId === 'null' || this._userId === '') {
      console.log('🔍 Tentando obter User ID do Mercado Livre...');
      try {
        const user = await this.getUserInfo();
        if (user && user.id) {
          this._userId = user.id.toString();
          console.log(`✅ User ID detectado: ${this._userId}`);
        }
      } catch (error: any) {
        console.warn('⚠️ Não foi possível obter User ID via API (Token pode estar inválido)');
        this._userId = process.env.ML_USER_ID || '';
      }
    }
    return this._userId;
  ...
  async request(endpoint: string, options: any = {}) {
    // Evita URLs malformadas com barras duplas se o ID estiver faltando
    const cleanEndpoint = endpoint.replace(/\/+/g, '/');
    if (cleanEndpoint.includes('//')) {
       console.error(`❌ URL Malformada detectada para ML: ${cleanEndpoint}`);
       throw new Error(`Invalid ML endpoint: ${cleanEndpoint}`);
    }
  ...
  async getListings(status: string = 'active', sort: string = 'date_desc', limit: number = 50, offset: number = 0) {
    try {
      ...
      if (!userId) {
        console.warn('⚠️ getListings: User ID do Mercado Livre não disponível. Pulando busca de anúncios.');
  ...
  async getQuestions(status: string = 'UNANSWERED', limit: number = 50) {
    try {
      const userId = await this.ensureUserId();
      if (!userId) {
        console.warn('⚠️ getQuestions: User ID do Mercado Livre não disponível. Pulando busca de perguntas.');
