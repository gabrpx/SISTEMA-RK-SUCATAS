...
// Configura o banco SQL local para gerenciar Autenticação Customizada 
// (uma vez que os provedores de Auth do Supabase foram desligados)
...
async function setupAdmin() {
  ...
  if (!adminExists) {
    const hash = await bcrypt.hash('rksucatas115935', 10);
 ...
 */
function cleanNotionId(id: string) {
  if (!id) return "";
 ...
 * Algumas versões da API exigem este formato na URL.
 */
...
// ID de fallback (Estoque) - Mantendo o último fornecido pelo usuário
const DATABASE_ID = cleanNotionId(process.env.NOTION_DATABASE_ID || "34adfa25a5288191a4eefb66d363f942");
const MOTOS_DATABASE_ID = cleanNotionId(process.env.NOTION_DB_MOTOS || "317dfa25a528805f9663ff0e6ebf0318");
const CLIENTS_DATABASE_ID = cleanNotionId(process.env.DATABASE_CLIENTES || process.env.NOTION_DB_CLIENTS || "");
const NOTION_VERSION = '2022-06-28'; // Estável por padrão
...
async function getCachedDbStructure(databaseId: string) {
  ...
  if (!cleanId) {
    console.error('❌ Erro: databaseId ausente no getCachedDbStructure');
    return { properties: {} };
  }
  ...
  console.log(`📡 Chamando Notion structure: ${url}`);
  ...
  if (!response.ok) {
    ...
    // Na versão 2025-09-03, se der erro de URL com UUID, tentamos o ID limpo
    if (errorBody.includes('invalid_request_url')) {
      console.log(`🔄 Erro de URL na estrutura. Tentando com ID limpo: ${cleanId}`);
    ...
    if (!response.ok) {
      ...
      console.error(`❌ Erro Notion Structure ${response.status}:`, finalError);
      if (response.status === 404) {
        console.warn(`⚠️ Estrutura do banco não encontrada (ID: ${cleanId}). Retornando estrutura vazia.`);
        return { properties: {} };
      }
      throw new Error(`Não foi possível carregar a estrutura do banco: ${response.statusText}. ${finalError.substring(0, 100)}`);
    }
  }
...
async function notionQuery(databaseId: string) {
  ...
  if (!cleanId || cleanId === 'undefined' || cleanId === 'null' || cleanId === '') {
    console.warn('⚠️ notionQuery: databaseId ausente ou inválido');
    return { results: [] };
  }
  
  // Para a versão 2025-09-03, usamos o ID limpo na URL
  const url = `https://api.notion.com/v1/databases/${cleanId}/query`;
  console.log(`🔍 Query Notion: ${url} (Versão: ${NOTION_VERSION})`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Se exigir versão nova (2025), tenta de novo com ela
      if (errorBody.includes('2025-09-03') || errorBody.includes('multiple_data_sources')) {
        console.log(`🔄 Banco exige versão nova. Tentando query com v2025-09-03...`);
      ...
      console.error(`❌ Erro Notion ${response.status} (banco: ${cleanId}):`, errorBody);
      throw new Error(`Notion API Error: ${errorBody}`);
    }

    return await response.json();
  } catch (err: any) {
    console.error("❌ Erro fatal na consulta Notion:", err.message);
    throw err;
  }
}
...
async function fetchAllFromNotion(databaseId: string) {
  ...
  if (!cleanId || cleanId === 'undefined' || cleanId === 'null') {
    console.warn('⚠️ fetchAllFromNotion: databaseId ausente ou inválido');
    return [];
  }
  ...
  if (cached && (Date.now() - cached.timestamp < DATA_CACHE_TTL)) {
    console.log(`📦 Usando cache para o banco ${cleanId}`);
    return cached.data;
  }

  // Se já houver uma busca em andamento para este banco, aguarda ela
  if (fetchLocks[cleanId]) {
    console.log(`⏳ Aguardando busca em andamento para o banco ${cleanId}`);
    return fetchLocks[cleanId];
  }
  ...
  const fetchPromise = (async () => {
    try {
      ...
      console.log(`🔄 Buscando TODAS as páginas do banco ${cleanId}`);
      ...
      while (hasMore) {
        ...
        if (!response.ok) {
          ...
          console.error(`❌ Erro Notion ${response.status} (banco: ${cleanId}):`, errorText);
          
          if (response.status === 404) {
            console.warn(`⚠️ Banco de dados não encontrado (ID: ${cleanId}). Verifique se ele foi compartilhado com a integração.`);
            return []; 
          }
          throw new Error(`Notion API error (${response.status}): ${errorText.substring(0, 200)}`);
        }
...
async function startServer() {
  ...
  const requiredEnv = [
    'NOTION_TOKEN', 'NOTION_DATABASE_ID', 'NOTION_DB_MOTOS', 'NOTION_DB_CLIENTS', 
    'DATABASE_VENDAS_ID', 'ML_CLIENT_ID', 'ML_CLIENT_SECRET', 'JWT_SECRET', 'FIREBASE_PROJECT_ID',
    'ML_ACCESS_TOKEN'
  ];
  
  requiredEnv.forEach(env => {
    if (!process.env[env]) {
      console.warn(`⚠️ Variável de ambiente [${env}] não está definida!`);
    } else {
      console.log(`✅ [${env}] está presente.`);
    }
  });

  if (process.env.APP_URL) {
    console.log(`ℹ️ Lembre-se de adicionar "${process.env.APP_URL}" aos domínios autorizados no Firebase console.`);
  }
  ...
  // ==================== AUTH ROUTES (CUSTOM LOCAL SQLITE) ====================
  ...
  // --- USUÁRIOS E CLIENTES (STAFF / LEAD) ---
  ...
  app.post("/api/usuarios", async (req, res) => {
    try {
      const { senha, ...restoDoBody } = req.body;
      ...
      // Sincronizar com Auth Local (SQLite)
      if (senha && payload.nome) {
        const cleanUser = payload.nome.trim().toLowerCase().replace(/\s+/g, '');
        const passwordHash = await bcrypt.hash(senha, 10);
        
        const exists = db.prepare("SELECT id FROM local_users WHERE username = ?").get(cleanUser);
        if (!exists) {
          db.prepare("INSERT INTO local_users (username, password_hash, role) VALUES (?, ?, ?)").run(cleanUser, passwordHash, payload.cargo || 'cliente');
        }
      }
  ...
  app.put("/api/usuarios/:id", async (req, res) => {
    try {
      ...
      const payload = { ...restoDoBody, atualizado_em: new Date().toISOString() };
      ...
      // Sincronizando com Auth Local (SQLite) na edição e promoção
      if (payload.nome) {
         const cleanUser = payload.nome.trim().toLowerCase().replace(/\s+/g, '');
         const exists = db.prepare("SELECT id FROM local_users WHERE username = ?").get(cleanUser);
         ...
         if (exists) {
           if (senha) {
             const passwordHash = await bcrypt.hash(senha, 10);
             db.prepare("UPDATE local_users SET role = ?, password_hash = ? WHERE username = ?").run(payload.cargo || 'cliente', passwordHash, cleanUser);
           } else {
             db.prepare("UPDATE local_users SET role = ? WHERE username = ?").run(payload.cargo || 'cliente', cleanUser);
           }
         } else if (senha) {
           // Se deram a ele uma senha e ele não existia no SQLITE, a gente cria!
           const passwordHash = await bcrypt.hash(senha, 10);
           db.prepare("INSERT INTO local_users (username, password_hash, role) VALUES (?, ?, ?)").run(cleanUser, passwordHash, payload.cargo || 'cliente');
         }
      }
  ...
  app.post("/api/produtos", async (req, res) => {
    try {
      const { 
        peca, nome, categoria, valor, moto, estoque, ano, rk_id, descricao, imagem, ml_link 
      } = req.body;
      
      let finalRkId = rk_id;

      // Lógica de ID Sequencial com Preenchimento de Lacunas
      if (!finalRkId) {
  ...
  app.post("/api/vendas/bulk-delete", async (req, res) => {
    try {
      const { ids } = req.body;
  ...
  app.post("/api/produtos/bulk-delete", async (req, res) => {
    try {
      const { ids } = req.body;
  ...
  app.post("/api/produtos/bulk-update-category", async (req, res) => {
    try {
  ...
  app.post("/api/produtos/bulk-update-stock", async (req, res) => {
    try {
...
function formatInventoryItem(page: any) {
  ...
  // 1. Título (Obrigatório no Notion, geralmente o nome do item)
  ...
  // 2. Mapeamento Inteligente (Baseado no modelo de Vendas)
  for (const [key, prop] of Object.entries(props)) {
    const value = prop as any;
    const lowerKey = key.toLowerCase();

    // Texto Rico (Descrição, Moto, Nome alternativo)
  ...
  app.delete("/api/sales/:id", async (req, res) => {
    try {
      ...
      const localSalesDbId = cleanNotionId(process.env.DATABASE_VENDAS_ID || "");
      invalidateCache(salesDbId);
      res.json({ success: true });
    } catch (error: any) {
  ...
  // Validar bancos de dados na inicialização
  const validateDatabases = async () => {
    const dbs = [
      { name: 'Estoque', id: DATABASE_ID },
      { name: 'Motos', id: MOTOS_DATABASE_ID },
      { name: 'Clientes', id: CLIENTS_DATABASE_ID },
      { name: 'Vendas', id: salesDbId }
    ];

    console.log('🧐 Iniciando validação dos bancos de dados do Notion...');

    for (const dbInfo of dbs) {
      if (!dbInfo.id) {
        console.warn(`⚠️ Banco [${dbInfo.name}] sem ID definido.`);
        continue;
      }
      try {
        const response = await fetch(`https://api.notion.com/v1/databases/${dbInfo.id}`, {
          headers: {
            'Authorization': `Bearer ${NOTION_TOKEN}`,
            'Notion-Version': NOTION_VERSION
          }
        ...
        if (response.ok) {
          const data = await response.json();
          const dbTitle = data.title?.[0]?.plain_text || 'Sem título';
          console.log(`✅ Conexão OK: [${dbInfo.name}] -> "${dbTitle}" (ID: ${dbInfo.id})`);
        } else {
          const errorText = await response.text();
          console.error(`❌ Erro no banco [${dbInfo.name}] (ID: ${dbInfo.id}): Status ${response.status}`);
          console.error(`   📝 Detalhe do Notion: ${errorText}`);
          if (response.status === 404) {
            console.error(`   💡 DICA: Vá no Notion > Banco ${dbInfo.name} > ... (topo dir) > Add Connections > RK_Sucatas`);
          }
        }
      } catch (err) {
        console.error(`❌ Falha crítica ao validar banco [${dbInfo.name}]:`, err);
      }
    }
  };
